...
def run_dataset(name, series, step, seq_len, horizons, out):
    ...
    for H in horizons:
        ...
        for k, fr in enumerate(fracs, 1):
            # ---------- honest (A) ----------
            sA = int(len(rows) * fr)
            teA_end = min(int(len(rows) * (fr + (1 - LAST_TRAIN))), len(rows))
...
if __name__ == '__main__':
    ...
    df = pd.DataFrame(out)
    df.to_csv('rolling_origin_aligned.csv', index=False)
    ...
    summ.to_csv('rolling_origin_aligned_summary.csv', index=False)
    print("\nSaved rolling_origin_aligned_summary.csv")
    ...
    fig.savefig('rolling_origin_aligned_fig.png', dpi=150,
                bbox_inches='tight')
